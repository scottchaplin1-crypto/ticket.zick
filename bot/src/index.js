...
import { trackStat } from "./handlers/achievements.js";
...
client.on(Events.MessageCreate, async (message) => {
  try {
    ...
    if (!handledAsQuickAdd) await tryHandleCustomCommand(message);
  } catch (err) {
    console.error("messageCreate handler error:", err.message);
  }

  if (!message.author.bot && message.guild) {
    trackStat(message.guild, message.author.id, "messages").catch(() => {});
  }
});
...
client.on(Events.MessageReactionAdd, (reaction, user) => {
  ...
  if (!user.bot && reaction.message?.guild) {
    trackStat(reaction.message.guild, user.id, "reactions").catch(() => {});
  }
});
...
client.on(Events.ThreadCreate, async (thread) => {
  try {
    ...
    await trackStat(thread.guild, thread.ownerId, "threads");
  } catch (err) {
    console.error("threadCreate tracking error:", err.message);
  }
});

client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
  try {
    // Went from not-boosting to boosting.
