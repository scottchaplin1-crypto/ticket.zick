...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (!message.author.bot && message.guild) {
    tryEarn(message.guild.id, message.author.id, "message").catch(() => {});
    if (messageHasGif(message)) {
      tryEarn(message.guild.id, message.author.id, "gif").catch(() => {});
    }
  }
});
...
client.on(Events.MessageReactionAdd, (reaction, user) => {
  ...
  if (!user.bot && reaction.message?.guild) {
    tryEarn(reaction.message.guild.id, user.id, "reaction").catch(() => {});
  }
});
...
client.on(Events.ThreadCreate, async (thread) => {
  try {
    ...
    await tryEarn(thread.guild.id, thread.ownerId, "thread");
  } catch (err) {
