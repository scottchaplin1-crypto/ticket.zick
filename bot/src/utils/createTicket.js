...
export async function createTicketChannel({ guild, client, panel, guildData, openerUser, extraUserId, answers }) {
  const globalStaffRoleIds = guildData.staffRoles.map((r) => r.roleId);
  const panelAccessRoleIds = JSON.parse(panel.accessRoleIds || "[]");
  // An empty list means "no override" — use every role from the global Staff Roles
  // page, same as before this feature existed. A non-empty list restricts this
  // specific panel's tickets to just those roles instead.
  const staffRoleIds = panelAccessRoleIds.length ? panelAccessRoleIds : globalStaffRoleIds;
  ...
  const openAddRoleIds = JSON.parse(panel.openAddRoleIds || "[]");
  if (openAddRoleIds.length) {
    try {
      const openerMember = await guild.members.fetch(openerUser.id);
      await openerMember.roles.add(openAddRoleIds);
    } catch (err) {
      console.error("Failed to add open-role(s) to ticket opener:", err.code || err.message);
    }
  }

  const welcomeEmbed = new EmbedBuilder()
    .setTitle(`Ticket #${ticket.number}`)
    .setDescription(panel.welcomeMessage || "Thanks for reaching out! Support will be with you shortly.")
    .setColor(panel.embedColor)
    .setFooter({ text: panel.footerText || "Powered by Ticket Zick" });
