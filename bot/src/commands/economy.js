...
export async function handleWork(interaction) {
  ...
  try {
    ...
    if (result.item) {
      ...
      await interaction.editReply(`🍀 **RARE FIND!** 💼 You worked hard and found... ${displayEmoji(result.item.emoji)} **${result.item.name}**!`);
      ...
      await interaction.editReply(
        `🍀 **RARE FIND!** 💼 You worked hard and earned a bonus **${result.currencyAmount} ${config?.currencyName || "currency"}**! (${result.newBalance} total)`
      );
    } else {
