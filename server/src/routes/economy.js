...
// A couple of routes are keyed by an item/outcome ID rather than a guild ID
// directly — these look up the owning guild first so the same gating logic
// still applies consistently.
const gateByItemGuild = requireSubscription(async (req) => {
  const item = await prisma.economyItem.findUnique({ where: { id: req.params.id } });
  return item?.guildId;
});
const gateByOutcomeGuild = requireSubscription(async (req) => {
  const outcome = await prisma.economyWorkOutcome.findUnique({ where: { id: req.params.id } });
  return outcome?.guildId;
});

// ---------- Config ----------

router.get("/guild/:guildId/config", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.put("/guild/:guildId/config", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.get("/guild/:guildId/items", requireAuth, requireSubscription("guildId"), async (req, res) => {
  const items = await prisma.economyItem.findMany({ where: { guildId: req.params.guildId }, orderBy: { price: "asc" } });
  res.json(items);
});

router.post("/guild/:guildId/items", requireAuth, requireSubscription("guildId"), async (req, res) => {
  const item = await prisma.economyItem.create({ data: { guildId: req.params.guildId, ...sanitizeItem(req.body) } });
  res.json(item);
});

router.patch("/items/:id", requireAuth, gateByItemGuild, async (req, res) => {
  const item = await prisma.economyItem.update({ where: { id: req.params.id }, data: sanitizeItem(req.body) });
  res.json(item);
});

router.delete("/items/:id", requireAuth, gateByItemGuild, async (req, res) => {
...
router.get("/guild/:guildId/balances", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.get("/guild/:guildId/purchases", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.get("/bot/:guildId/config", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
  const config = await prisma.economyConfig.findUnique({ where: { guildId: req.params.guildId } });
...
router.get("/bot/:guildId/items", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
  const items = await prisma.economyItem.findMany({ where: { guildId: req.params.guildId, enabled: true }, orderBy: { price: "asc" } });
  res.json(items);
});

router.get("/bot/:guildId/balance/:userId", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
...
router.post("/bot/:guildId/balance/:userId/add", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
...
router.post("/bot/:guildId/purchase", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
...
router.get("/guild/:guildId/work-config", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.put("/guild/:guildId/work-config", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.get("/guild/:guildId/work-outcomes", requireAuth, requireSubscription("guildId"), async (req, res) => {
  const outcomes = await prisma.economyWorkOutcome.findMany({ where: { guildId: req.params.guildId }, orderBy: { chanceOneInN: "desc" } });
  res.json(outcomes);
});

router.post("/guild/:guildId/work-outcomes", requireAuth, requireSubscription("guildId"), async (req, res) => {
  const outcome = await prisma.economyWorkOutcome.create({ data: { guildId: req.params.guildId, ...sanitizeOutcome(req.body) } });
  res.json(outcome);
});

router.patch("/work-outcomes/:id", requireAuth, gateByOutcomeGuild, async (req, res) => {
  const outcome = await prisma.economyWorkOutcome.update({ where: { id: req.params.id }, data: sanitizeOutcome(req.body) });
  res.json(outcome);
});

router.delete("/work-outcomes/:id", requireAuth, gateByOutcomeGuild, async (req, res) => {
...
router.get("/bot/:guildId/work-config", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
  const config = await prisma.economyWorkConfig.findUnique({ where: { guildId: req.params.guildId } });
  res.json(config || { enabled: false, cooldownSeconds: 3600, baseMin: 10, baseMax: 50 });
});
...
router.post("/bot/:guildId/work", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
...
router.get("/guild/:guildId/earn-rules", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.put("/guild/:guildId/earn-rules/:type", requireAuth, requireSubscription("guildId"), async (req, res) => {
...
router.post("/bot/:guildId/earn", requireBotSecret, requireSubscription("guildId"), async (req, res) => {
