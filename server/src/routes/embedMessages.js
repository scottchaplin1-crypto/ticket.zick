...
router.post("/:id/send", requireAuth, gateByMsgGuild, async (req, res) => {
  ...
  try {
    ...
    if (msg.channelId === channelId && msg.messageId) {
      // Same channel as last time — edit that message instead of posting a
      // duplicate. Falls back to posting fresh if the edit fails for any
      // reason — most commonly because the message was deleted manually in
      // Discord, which otherwise looked exactly like a permission error even
      // though permissions were never the actual problem.
      try {
