...
// Case-insensitive on purpose — WOM returns rank strings in whatever case their
// API uses, which won't always match how someone typed it into the mapping on
// the dashboard (e.g. "hellcat" from WOM vs "Hellcat" typed in). A mismatch here
// silently means that rank never gets a role at all, so this is forgiving the
// same way RSN matching already is. Shared by both the initial-link flow and the
// periodic sync, so a fix here can't drift out of sync between the two again.
function roleForRank(mappings, rank) {
  return mappings.find((m) => (m.womRole || "").trim().toLowerCase() === (rank || "").trim().toLowerCase())?.roleId;
}
