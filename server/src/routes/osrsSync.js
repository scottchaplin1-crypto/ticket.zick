...
router.get("/guild/:guildId/links", requireAuth, async (req, res) => {
  ...
  // Checked one at a time on purpose — firing all of these at once risks Discord
  // rate-limiting a few of them, which was being misread as "they left the server"
  // when it was really just a transient fetch failure.
  const enriched = [];
