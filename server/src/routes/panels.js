...
router.post("/:panelId/send", requireAuth, async (req, res) => {
  ...
  if (panel.buttonEmoji) button.emoji = { name: panel.buttonEmoji };

  try {
    const { data: message } = await botApi.post(`/channels/${channelId}/messages`, {
      embeds: [embed],
...
function sanitize(body) {
  const allowed = [
    ...
    "ticketCategoryId", "namingPattern", "maxOpenPerUser", "pingRoleIds", "tagStaffOnOpen",
