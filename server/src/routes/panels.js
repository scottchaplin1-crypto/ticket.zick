...
router.post("/:panelId/send", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (panel.channelId === channelId && panel.messageId) {
      ...
      // Messages already do. Falls back to posting fresh if the edit fails for
      // any reason — most commonly because the message was deleted manually in
...
function sanitize(body) {
  const allowed = [
    ...
    "welcomeMessage", "footerText", "openAddRoleIds", "closeAddRoleIds", "closeRemoveRoleIds",
