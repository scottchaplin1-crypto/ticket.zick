...
// A few routes are keyed by the giveaway's own ID rather than a guild ID
// directly — this looks up the owning guild first so the same gating still applies.
const gateByGiveawayGuild = requireSubscription(async (req) => {
...
router.patch("/bot/:id/message", requireBotSecret, gateByGiveawayGuild, async (req, res) => {
  const giveaway = await prisma.giveaway.update({ where: { id: req.params.id }, data: { messageId: req.body.messageId } });
  res.json(giveaway);
});

router.get("/bot/:id", requireBotSecret, gateByGiveawayGuild, async (req, res) => {
  const giveaway = await prisma.giveaway.findUnique({ where: { id: req.params.id } });
...
router.post("/bot/:id/toggle-entry", requireBotSecret, gateByGiveawayGuild, async (req, res) => {
...
router.post("/bot/:id/end", requireBotSecret, gateByGiveawayGuild, async (req, res) => {
  const giveaway = await prisma.giveaway.findUnique({ where: { id: req.params.id } });
  if (!giveaway) return res.status(404).json({ error: "Not found" });
