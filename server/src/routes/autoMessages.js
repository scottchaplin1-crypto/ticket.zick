...
// Keyed by the message's own ID rather than a guild ID directly.
const gateByMsgGuild = requireSubscription(async (req) => {
  const msg = await prisma.autoMessage.findUnique({ where: { id: req.params.id } });
  return msg?.guildId;
});
...
router.patch("/:id", requireAuth, gateByMsgGuild, async (req, res) => {
  const message = await prisma.autoMessage.update({ where: { id: req.params.id }, data: sanitize(req.body) });
  res.json(message);
});

router.delete("/:id", requireAuth, gateByMsgGuild, async (req, res) => {
...
router.post("/:id/send-now", requireAuth, gateByMsgGuild, async (req, res) => {
...
export async function runAutoMessageSweep() {
  ...
  for (const message of due) {
    ...
    // Unlike the HTTP routes above, this runs on a timer with no request to
    // gate — without this check, a message set up while subscribed would just
    // keep posting forever even after the subscription lapsed.
    if (!(await isGuildSubscribed(message.guildId))) continue;
