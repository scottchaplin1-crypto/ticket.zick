...
// Some routes are keyed by the panel's own ID, or by a Discord message ID,
// rather than a guild ID directly — these look up the owning guild first.
const gateByPanelGuild = requireSubscription(async (req) => {
  const panel = await prisma.reactionRolePanel.findUnique({ where: { id: req.params.panelId } });
  return panel?.guildId;
});
const gateByMessageGuild = requireSubscription(async (req) => {
...
router.post("/guild/:guildId", requireAuth, requireSubscription("guildId"), async (req, res) => {
  const panel = await prisma.reactionRolePanel.create({ data: { guildId: req.params.guildId, ...sanitize(req.body) } });
  res.json(panel);
});

router.patch("/:panelId", requireAuth, gateByPanelGuild, async (req, res) => {
  const panel = await prisma.reactionRolePanel.update({ where: { id: req.params.panelId }, data: sanitize(req.body) });
  res.json(panel);
});

router.delete("/:panelId", requireAuth, gateByPanelGuild, async (req, res) => {
...
router.post("/:panelId/send", requireAuth, gateByPanelGuild, async (req, res) => {
...
router.get("/bot/:panelId", requireBotSecret, gateByPanelGuild, async (req, res) => {
  const panel = await prisma.reactionRolePanel.findUnique({ where: { id: req.params.panelId } });
...
router.get("/bot/by-message/:messageId", requireBotSecret, gateByMessageGuild, async (req, res) => {
  const panel = await prisma.reactionRolePanel.findFirst({ where: { messageId: req.params.messageId } });
  if (!panel) return res.status(404).json({ error: "Not found" });
  res.json(panel);
});
