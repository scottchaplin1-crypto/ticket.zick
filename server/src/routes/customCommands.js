...
// Keyed by the command's own ID rather than a guild ID directly.
const gateByCommandGuild = requireSubscription(async (req) => {
  const command = await prisma.customCommand.findUnique({ where: { id: req.params.commandId } });
...
router.patch("/:commandId", requireAuth, gateByCommandGuild, async (req, res) => {
  try {
...
router.delete("/:commandId", requireAuth, gateByCommandGuild, async (req, res) => {
