...
router.get("/guild/:guildId/status", requireAuth, async (req, res) => {
  ...
  let sub = await prisma.subscription.findUnique({ where: { guildId } });

  // This endpoint is only hit when someone actually opens the Billing page —
  // low enough traffic that checking Stripe live here (rather than trusting
  // whatever our cached status says) is cheap, and it means a cancellation made
  // directly in Stripe shows up the moment someone looks, not up to an hour
  // later when the background sweep next runs.
  if (sub?.stripeSubscriptionId) {
    try {
      const stripeSub = await stripe.subscriptions.retrieve(sub.stripeSubscriptionId);
      if (stripeSub.status !== sub.status) {
        sub = await prisma.subscription.update({ where: { id: sub.id }, data: { status: stripeSub.status } });
      }
    } catch (err) {
      console.error(`Live status check failed for guild ${guildId}:`, err.message);
      // Falls through and uses whatever we had cached — better to show
      // possibly-stale data than nothing at all if Stripe is unreachable.
    }
  }
