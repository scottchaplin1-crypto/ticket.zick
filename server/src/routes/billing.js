...
router.get("/guild/:guildId/status", requireAuth, async (req, res) => {
  ...
  // Fast path — just reads the cached value, kept fresh by the webhook and the
  // 15-minute sweep. Called constantly (every gated page, the sidebar on every
  // guild switch), so it deliberately does NOT hit Stripe live — that cost is
  // reserved for the explicit /refresh endpoint below, called only when someone
  // is actually looking at the Billing page.
  const sub = await prisma.subscription.findUnique({ where: { guildId } });
  const subscribed = sub?.status === "active" || sub?.status === "trialing";
  res.json({ subscribed, exempt: false, status: sub?.status || "inactive" });
});

// Only the Billing page calls this — checks Stripe live and updates the cached
// value, so someone actively looking at their subscription status always sees
// something accurate, without paying that cost on every gated page everywhere.
router.post("/guild/:guildId/refresh", requireAuth, async (req, res) => {
  const { guildId } = req.params;
  if (isExemptGuild(guildId)) return res.json({ ok: true });

  const sub = await prisma.subscription.findUnique({ where: { guildId } });
  if (!sub?.stripeSubscriptionId) return res.json({ ok: true });
  ...
  try {
    ...
    console.error(`Live refresh failed for guild ${guildId}:`, err.message);
    res.json({ ok: true }); // non-fatal — the Billing page just shows whatever was cached
  }
});
