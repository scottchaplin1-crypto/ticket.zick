...
import {
  ...
  LogIn, SmilePlus, Terminal, Ticket, Wrench, ChevronDown, Bot, LayoutTemplate, Swords,
} from "lucide-react";
...
const BOT_INVITE_PERMISSIONS = 1342295056; // includes Manage Guild Expressions, for reading/using custom emojis
...
const GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "osrs-sync", label: "OSRS Rank Sync", icon: Swords },
                ...
                <div className="flex flex-col gap-1 mt-1">
                  {group.items.map((item) =>
                    item.isPanels ? (
                      ...
                      <NavLink key={item.to} to={`${base}/${item.to}`} onClick={(e) => handleNavClick(e, item.isPanels ? panelsHref : `${base}/${item.to}`)} className={({ isActive }) => itemClass(isActive)}>
                        <item.icon size={16} strokeWidth={2} />
                        {item.label}
                      </NavLink>
                    )
