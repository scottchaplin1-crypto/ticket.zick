...
import {
  ...
  LogIn, SmilePlus, Terminal, Ticket, Wrench, ChevronDown,
} from "lucide-react";
...
export default function Sidebar({ guildId, guildName }) {
  ...
  // Guards any in-app navigation triggered from the sidebar — if the page you're
  // leaving has unsaved changes, confirm before discarding them.
  function guardNav(e) {
  ...
  return (
    <div className="w-64 shrink-0 bg-surface2 min-h-screen p-4 flex flex-col border-r border-white/5">
                ...
                <div className="flex flex-col gap-1 mt-1">
                  {group.items.map((item) =>
                    item.isPanels ? (
                      ...
                      <NavLink key={item.to} to={`${base}/${item.to}`} onClick={guardNav} className={({ isActive }) => itemClass(isActive)}>
                        <item.icon size={16} strokeWidth={2} />
                        {item.label}
                      </NavLink>
                    )
      ...
      <div className="mt-auto pt-4 border-t border-white/5">
        <a href="/" onClick={guardNav} className="text-xs text-gray-500 hover:text-gray-300 px-1">
          ← All servers
        </a>
      </div>
    </div>
