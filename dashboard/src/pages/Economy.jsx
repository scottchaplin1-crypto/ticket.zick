...
export default function Economy({ guildId }) {
  return (
    <PremiumGate guildId={guildId} feature="Economy">
      <EconomyContent guildId={guildId} />
    </PremiumGate>
  );
}

function EconomyContent({ guildId }) {
  ...
  const [newOutcome, setNewOutcome] = useState({ label: "", chanceOneInN: 128, mode: "currency", currencyMin: 50, currencyMax: 200, itemId: "" });
  ...
  useEffect(() => {
    api
      .get(`/api/economy/guild/${guildId}/config`)
      .then((res) => {
        setConfig(res.data);
      ...
      .catch(() => {
        ...
        // guild — falls back to a blank config so the page can still render
        // (dimmed, via PremiumGate) instead of hanging on "Loading…" forever.
        const blank = { enabled: false, currencyName: "Coins", currencyEmoji: "🪙", collectionLogChannelId: null };
        setConfig(blank);
        configBaselineRef.current = JSON.stringify(blank);
      });
    ...
    api.get(`/api/economy/guild/${guildId}/balances`).then((res) => setBalances(res.data)).catch(() => setBalances([]));
    api.get(`/api/economy/guild/${guildId}/purchases`).then((res) => setPurchases(res.data)).catch(() => setPurchases([]));
    api
      .get(`/api/economy/guild/${guildId}/earn-rules`)
      .then((res) => setEarnRules(res.data))
      .catch(() => setEarnRules(EARN_TYPE_LABELS.map(({ type }) => ({ type, enabled: false, amountMin: 1, amountMax: 1, cooldownSeconds: 60 }))));
    api
      .get(`/api/economy/guild/${guildId}/work-config`)
      .then((res) => setWorkConfig(res.data))
      .catch(() => setWorkConfig({ enabled: false, cooldownSeconds: 3600, baseMin: 10, baseMax: 50 }));
    loadOutcomes().catch(() => setOutcomes([]));
  }, [guildId]);
  ...
  async function addOutcome() {
    const payload = {
      label: newOutcome.label || (newOutcome.mode === "item" ? items.find((i) => i.id === newOutcome.itemId)?.name : "Rare bonus"),
      chanceOneInN: newOutcome.chanceOneInN,
      currencyMin: newOutcome.mode === "currency" ? newOutcome.currencyMin : null,
      currencyMax: newOutcome.mode === "currency" ? newOutcome.currencyMax : null,
      itemId: newOutcome.mode === "item" ? newOutcome.itemId : null,
      enabled: true,
    };
    await api.post(`/api/economy/guild/${guildId}/work-outcomes`, payload);
    await loadOutcomes();
    setNewOutcome({ label: "", chanceOneInN: 128, mode: "currency", currencyMin: 50, currencyMax: 200, itemId: "" });
  }
    ...
    <div className="grid grid-cols-2 gap-6 items-start">
      <Card>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg font-medium disabled:opacity-50 text-sm"
          >
      ...
      <Card>
        ...
        <div className="space-y-2 text-sm text-gray-400">
          ...
          <p><code className="text-gray-200">!buy item name</code> — purchase something</p>
          <p><code className="text-gray-200">!work</code> — work for a chance at currency or items</p>
          <p><code className="text-gray-200">/give-currency user amount</code> — staff-only slash command, manually grant or remove currency</p>
          <p className="text-xs text-gray-500 pt-1">
            The everyday commands use <code>!</code> instead of <code>/</code> — snappier for repeated use. Only the
            staff-management command stays a proper slash command, restricted by Discord's own permissions.
          </p>
        </div>
      </Card>
    </div>
              ...
              <div key={type} className="bg-surface3 rounded-lg p-3">
                ...
                {rule.enabled && (
                  <div className="grid grid-cols-3 gap-2">
                    <label className="block">
                      <span className="block text-[11px] text-gray-500 mb-1">Min amount</span>
                      <input
                        type="number"
                        min={0}
                        className="input h-9 text-sm"
                        value={rule.amountMin}
                        onChange={(e) => updateEarnRule(type, { amountMin: e.target.value })}
                      />
                    </label>
                    <label className="block">
                      <span className="block text-[11px] text-gray-500 mb-1">Max amount</span>
                      <input
                        type="number"
                        min={0}
                        className="input h-9 text-sm"
                        value={rule.amountMax}
                        onChange={(e) => updateEarnRule(type, { amountMax: e.target.value })}
                      />
                    </label>
                    <label className="block">
      ...
      {workConfig && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              <span className="flex items-center gap-1.5 text-xs text-gray-400 mb-1">
                Guaranteed reward: min
                <Tooltip text="Everyone gets a random amount in this range every time they work, unless a rare outcome below hits instead.">
                  <Info size={12} className="text-gray-600 hover:text-cyan-400 transition cursor-help" />
                </Tooltip>
              </span>
              <input
                ...
                min={0}
                className="input"
                value={workConfig.baseMin}
                onChange={(e) => { setWorkConfig({ ...workConfig, baseMin: e.target.value }); setWorkSaved(false); }}
              />
            </label>
            <label className="block">
              ...
              <input
                ...
                min={0}
                className="input"
                value={workConfig.baseMax}
                onChange={(e) => { setWorkConfig({ ...workConfig, baseMax: e.target.value }); setWorkSaved(false); }}
              />
            </label>
          </div>
          {workSaved && <p className="text-xs text-green-400">Saved.</p>}
          <button onClick={saveWorkConfig} disabled={workSaving} className="w-full py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg font-medium disabled:opacity-50 text-sm">
            {workSaving ? "Saving…" : "Save /work settings"}
          </button>

          <div className="pt-3 border-t border-white/5">
            <p className="text-xs text-gray-400 mb-2">
              Rare outcomes — each one is its own independent "1 in N" chance, checked before the guaranteed
              reward above. Just like OSRS: the rarest ones are tried first, and if none hit, the normal reward
              applies.
            </p>
                ...
                <div key={o.id} className={`flex items-center justify-between bg-surface3 rounded-lg p-2 ${!o.enabled ? "opacity-50" : ""}`}>
                  <div className="text-sm">
                    ...
                    <span className="text-gray-500 ml-2 text-xs">
                      {o.itemId ? "item" : `${o.currencyMin}-${o.currencyMax} ${config.currencyName}`} · 1 in {o.chanceOneInN}
                    </span>
                  </div>
            ...
            <div className="bg-surface3 rounded-lg p-3 space-y-2">
              <div className="grid grid-cols-2 gap-2 items-center">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-xs text-gray-500 shrink-0">1 in</span>
                  <input
                    type="number"
                    min={1}
                    className="input h-10 text-sm"
                    placeholder="128"
                    value={newOutcome.chanceOneInN}
                    onChange={(e) => setNewOutcome({ ...newOutcome, chanceOneInN: e.target.value })}
                  />
                </div>
              </div>
              {newOutcome.mode === "currency" ? (
                <>
                  <input
                    className="input h-10 text-sm"
                    placeholder="Label (e.g. Rare bonus)"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      value={newOutcome.currencyMin}
                      onChange={(e) => setNewOutcome({ ...newOutcome, currencyMin: e.target.value })}
                    />
                    <input
                      type="number"
                      min={0}
                      ...
                      value={newOutcome.currencyMax}
                      onChange={(e) => setNewOutcome({ ...newOutcome, currencyMax: e.target.value })}
                    />
                  </div>
                </>
              ) : (
              ...
              <button
                ...
                className="w-full py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg text-sm font-medium disabled:opacity-50"
              >
                Add outcome
              </button>
            </div>
  ...
  const storeTab = (
    <div>
      ...
      <div className="flex justify-end mb-4">
        <button onClick={newItem} className="flex items-center gap-1.5 px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg text-sm font-medium">
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-2">Economy</h1>
      <p className="text-sm text-gray-500 mb-6">
        A custom server currency members can earn and spend. Set up your currency's name, how people earn it,
        the <code className="text-gray-400">/work</code> command, and what's in the store — organized into the
        tabs below.
      </p>
      <Tabs
        tabs={[
          { id: "settings", label: "Settings", icon: Coins, content: settingsTab },
          { id: "earning", label: "Earning", icon: Trophy, content: earningTab },
          { id: "work", label: "Work", icon: Briefcase, content: workTab },
          { id: "store", label: "Store", icon: ShoppingBag, content: storeTab },
        ]}
      />
    </div>
  );
