...
export default function CustomCommands({ guildId }) {
  return (
    <PremiumGate guildId={guildId} feature="Custom Commands">
      <CustomCommandsContent guildId={guildId} />
    </PremiumGate>
  );
}

function CustomCommandsContent({ guildId }) {
  ...
  // The actual state change, with no guard — used both by the guarded public
  // select()/newCommand() below AND right after a successful save (where there's
  // nothing to "discard", so routing that through the guard would be a bug).
  function applySelection(cmd) {
    setSelected(cmd?.id ?? null);
    setForm(cmd || BLANK);
    baselineRef.current = JSON.stringify(cmd || BLANK);
    setError("");
  }

  function select(cmd) {
    requestNavigation(() => applySelection(cmd));
  }
  ...
  async function remove(id) {
    ...
    if (selected === id) applySelection(null);
    load();
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={newCommand} className="flex items-center gap-1.5 px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg text-sm font-medium">
        ...
        <Card title={selected ? "Edit command" : "New command"}>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <ImageUrlField label="Image URL" value={form.embedImageUrl} onChange={(v) => setForm({ ...form, embedImageUrl: v })} />
            </div>
            <ImageUrlField label="Thumbnail URL" value={form.embedThumbnailUrl} onChange={(v) => setForm({ ...form, embedThumbnailUrl: v })} />
            ...
            <button
              onClick={save}
              disabled={saving}
              className="w-full py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg font-medium disabled:opacity-50"
            >
