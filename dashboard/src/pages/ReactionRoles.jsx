...
export default function ReactionRoles({ guildId }) {
  ...
  async function save() {
    ...
    try {
      ...
      await load();
      setDirty(false); // just saved — select() below would otherwise think this is a discard
  ...
  // Autosaves once an existing panel is selected — a brand new panel still needs the
  // explicit "Create panel" click, since there's nothing to patch yet.
  const saveStatus = useAutoSave(form, () => api.patch(`/api/reaction-roles/${selected}`, toPayload()).then(load), {
    enabled: !!selected,
    resetKey: selected,
  });

  // A brand new, not-yet-created panel isn't covered by autosave, so track it
  // manually — editing an existing one relies on autosave instead.
  const { isDirty, setDirty } = useUnsavedChanges();
  useEffect(() => {
    setDirty(!selected && JSON.stringify(form) !== JSON.stringify(BLANK));
  }, [form, selected]);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          {!selected && isDirty && <p className="text-xs text-amber-400">You have unsaved changes</p>}
