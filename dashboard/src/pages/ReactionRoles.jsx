...
const BLANK = {
  ...
  color: "#5865F2",
  mode: "multiple",
  deliveryMethod: "reaction",
  mappings: [],
};

const CREATE_ROLE_VALUE = "__create__";

// Discord only offers these four button colors — no custom hex, same limit as
// the ticket panel's own button style.
const BUTTON_STYLES = [
  { value: "Primary", label: "Blurple", color: "#5865F2" },
  { value: "Secondary", label: "Gray", color: "#4E5058" },
  { value: "Success", label: "Green", color: "#23A55A" },
  { value: "Danger", label: "Red", color: "#DA373C" },
];
...
export default function ReactionRoles({ guildId }) {
  return (
    <PremiumGate guildId={guildId} feature="Reaction Roles">
      <ReactionRolesContent guildId={guildId} />
    </PremiumGate>
  );
}

function ReactionRolesContent({ guildId }) {
  ...
  function addMapping() {
    setForm({ ...form, mappings: [...form.mappings, { emoji: "⭐", roleId: "", label: "", style: "Secondary" }] });
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={newPanel} className="flex items-center gap-1.5 px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg text-sm font-medium">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Let members pick up roles by clicking a reaction on a message — no commands needed.
      </p>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <label className="block">
                  <span className="flex items-center gap-1.5 text-xs text-gray-400 mb-1">
                    Mode
                    <Tooltip text="Multiple: people can pick as many as they like, holding several roles at once. Single: picking a new one automatically removes whichever they had before from this same panel.">
                      <Info size={12} className="text-gray-600 hover:text-cyan-400 transition cursor-help" />
                    </Tooltip>
                  </span>
                  <Select
                ...
                <label className="block">
                  <span className="flex items-center gap-1.5 text-xs text-gray-400 mb-1">
                    Delivery method
                    <Tooltip text="Reactions: people click an emoji reaction on the message — classic, but relies on them knowing what each emoji means. Buttons: real clickable buttons showing each role's name directly — more obvious, and easier on mobile. Both work the same way once picked — click to add, click again to remove.">
                      <Info size={12} className="text-gray-600 hover:text-cyan-400 transition cursor-help" />
                    </Tooltip>
                  </span>
                  <Select
                    ...
                    options={[
                      { value: "reaction", label: "Reactions — click an emoji" },
                      { value: "button", label: "Buttons — click a labeled button" },
                    ]}
                  />
                </label>
              </div>
            ...
            <div className="space-y-2">
              {form.mappings.map((m, i) => (
                <div key={i} className="bg-surface3 rounded-lg p-2 space-y-2">
                  ...
                  {form.deliveryMethod === "button" && (
                    <div className="flex items-center gap-2 pl-[4.5rem]">
                      <span className="text-xs text-gray-500 shrink-0">Button color</span>
                      <div className="flex items-center gap-1.5">
                        {BUTTON_STYLES.map((s) => (
                          <button
                            key={s.value}
                            onClick={() => updateMapping(i, "style", s.value)}
                            title={s.label}
                            className={`w-6 h-6 rounded-full transition ${(m.style || "Secondary") === s.value ? "ring-2 ring-offset-2 ring-offset-surface3 ring-cyan-400" : "opacity-60 hover:opacity-100"}`}
        ...
        <div className="sticky top-6">
          <Card title="Live preview">
            <div className="bg-[#313338] rounded-lg p-4 border-l-4" style={{ borderColor: form.color }}>
              ...
              {form.deliveryMethod === "reaction" && form.mappings.length > 0 && (
              ...
              {form.deliveryMethod === "reaction" ? (
                ...
                <div className="mt-3 flex gap-1.5 flex-wrap">
                  {form.mappings.map((m, i) => (
                    <span
                      key={i}
                      className="px-3 py-1.5 rounded text-white text-sm font-medium flex items-center gap-1.5"
                      style={{ backgroundColor: BUTTON_STYLES.find((s) => s.value === (m.style || "Secondary"))?.color }}
                    >
                      <EmojiDisplay value={m.emoji} />
                      {m.label || roles.find((r) => r.id === m.roleId)?.name || "Role"}
                    </span>
                  ))}
                  {form.mappings.length === 0 && (
                    <span className="text-xs text-gray-500 flex items-center gap-1.5">
                      <Smile size={12} /> Add role mappings to see buttons here
                    </span>
                  )}
                  {form.mappings.length > 25 && (
                    <p className="text-xs text-amber-400 w-full mt-1">
                      Discord allows a maximum of 25 buttons — trim this down before sending, or switch to reactions.
                    </p>
                  )}
                </div>
              )}
