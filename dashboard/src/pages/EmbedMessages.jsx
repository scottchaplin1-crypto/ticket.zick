...
export default function EmbedMessages({ guildId }) {
  return (
    <PremiumGate guildId={guildId} feature="Embed Messages">
      <EmbedMessagesContent guildId={guildId} />
    </PremiumGate>
  );
}

function EmbedMessagesContent({ guildId }) {
  ...
  function applySelection(msg) {
    ...
    const data = msg ? { ...msg, embeds: JSON.parse(msg.embeds || "[]") } : BLANK;
    setForm(data);
  ...
  async function save() {
    ...
    try {
      if (selected) {
        await api.patch(`/api/embed-messages/${selected}`, toPayload());
        await load();
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-6 items-start">
        <div className="space-y-4">
          ...
          >
            {saving ? "Saving…" : selected ? "Save changes" : "Create message"}
          </button>
          {selected && (
            ...
            <div className="bg-[#313338] rounded-lg p-4">
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0 space-y-3">
                  ...
                  {form.content && <p className="text-gray-100 text-sm whitespace-pre-wrap -mt-1">{renderRichText(form.content)}</p>}

                  {form.embeds.map((embed) => (
                    <div key={embed.id} className="border-l-4 rounded bg-black/20 p-3" style={{ borderColor: embed.color || "#5865F2" }}>
                      {embed.authorName && <p className="text-xs text-gray-300 font-medium mb-1">{renderRichText(embed.authorName)}</p>}
                      {embed.title && <p className="text-white font-bold text-sm">{renderRichText(embed.title)}</p>}
                      {embed.description && <p className="text-gray-300 text-sm mt-1 whitespace-pre-wrap">{renderRichText(embed.description)}</p>}
                      {embed.fields?.length > 0 && (
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          {embed.fields.map((f, i) => (
                            <div key={i} className={f.inline ? "" : "col-span-2"}>
                              <p className="text-xs font-semibold text-gray-200">{renderRichText(f.name)}</p>
                              <p className="text-xs text-gray-400">{renderRichText(f.value)}</p>
                            </div>
                          ))}
                        </div>
                      )}
                      {embed.imageUrl && <img src={embed.imageUrl} className="mt-2 rounded max-h-40 object-cover w-full" />}
                      {embed.footerText && <p className="text-gray-500 text-xs mt-2">{renderRichText(embed.footerText)}</p>}
                    </div>
                  ))}
