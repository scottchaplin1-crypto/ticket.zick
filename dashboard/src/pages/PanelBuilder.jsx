...
export default function PanelBuilder({ guildId }) {
  ...
  // Only autosaves once you're editing an existing panel — a brand new, unsaved
  // panel still needs the explicit "Create panel" click, since there's nothing to
  // patch yet and we don't want to silently create panels from a half-finished form.
  const saveStatus = useAutoSave(form, () => api.patch(`/api/panels/${panelId}`, toPayload()).then(loadPanels), {
    enabled: loaded && !!panelId,
    resetKey: panelId,
  });

  // A brand new, not-yet-created panel isn't covered by autosave, so track it
  // manually — editing an existing panel relies on autosave instead (see above),
  // which keeps the "unsaved changes" warning from firing during that ~1s gap.
  const { setDirty, requestNavigation } = useUnsavedChanges();
  useEffect(() => {
    if (!loaded) return;
    setDirty(!panelId && JSON.stringify(form) !== JSON.stringify(BLANK));
  }, [form, panelId, loaded]);
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-4">
          {panelId && <SaveStatus status={saveStatus} />}
          <button
