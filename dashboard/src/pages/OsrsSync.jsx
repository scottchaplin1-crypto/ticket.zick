...
export default function OsrsSync({ guildId }) {
  return (
    <PremiumGate guildId={guildId} feature="OSRS Rank Sync">
      <OsrsSyncContent guildId={guildId} />
    </PremiumGate>
  );
}

function OsrsSyncContent({ guildId }) {
  ...
  useEffect(() => {
    api
      .get(`/api/osrs-sync/guild/${guildId}`)
      .then((res) =>
        setForm({
          ...BLANK,
          ...res.data,
        ...
        <div className="space-y-4">
          <Card>
            ...
            <div className="space-y-3">
              ...
              <button onClick={save} disabled={saving} className="w-full py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg font-medium disabled:opacity-50">
                {saving ? "Saving…" : "Save"}
              </button>
            </div>
          </Card>
          ...
          <Card title="Rank → Role mappings">
            ...
            <button onClick={save} disabled={saving} className="w-full mt-3 py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg font-medium disabled:opacity-50 text-sm">
          ...
          <div className="bg-surface2 border border-white/5 rounded-xl p-4 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={sendPanel} disabled={!sendChannelId || sending} className="px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg text-sm font-medium disabled:opacity-40 whitespace-nowrap">
        ...
        <div className="sticky top-6">
          <Card title="Live preview">
            <div className="bg-[#313338] rounded-lg p-4 border-l-4" style={{ borderColor: "#5865F2" }}>
              ...
              <button className="mt-4 px-4 py-2 rounded bg-cyan-500 text-[#0b1416] shadow-md shadow-cyan-500/20 text-sm font-medium">⚔️ Link RSN</button>
            </div>
          </Card>
        </div>
      </div>
