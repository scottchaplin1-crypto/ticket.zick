...
export default function BotProfile({ guildId }) {
  return (
    <PremiumGate guildId={guildId} feature="Bot Profile">
      <BotProfileContent guildId={guildId} />
    </PremiumGate>
  );
}

function BotProfileContent({ guildId }) {
  ...
  useEffect(() => {
    api
      .get(`/api/bot-profile/guild/${guildId}`)
      .then((res) => {
        setForm(res.data);
      ...
      .catch(() => {
        ...
        // guild — falls back to a blank profile so the page can still render
        // instead of hanging forever.
        const blank = { nickname: "", avatarUrl: "", bannerUrl: "", bio: "" };
        setForm(blank);
