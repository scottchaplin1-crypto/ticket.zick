...
export default function Moderation({ guildId }) {
  return (
    <PremiumGate guildId={guildId} feature="Moderation">
      <ModerationContent guildId={guildId} />
    </PremiumGate>
  );
}

function ModerationContent({ guildId }) {
  ...
  useEffect(() => {
    api
      .get(`/api/moderation/guild/${guildId}`)
      .then((res) => {
        setForm(res.data);
      ...
      .catch(() => {
        ...
        const blank = { modLogChannelId: null, autoBanEnabled: false, autoBanMinAgeDays: 7 };
        setForm(blank);
    ...
    api.get(`/api/moderation/guild/${guildId}/cases`).then((res) => setCases(res.data)).catch(() => setCases([]));
  }, [guildId]);
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-6">
        Ban and kick commands, plus automatic protection against very new accounts joining to raid.
      </p>
