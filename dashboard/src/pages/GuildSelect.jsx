...
import AdminInviteNote from "../components/AdminInviteNote.jsx";
    ...
    <div className="min-h-screen max-w-4xl mx-auto py-16 px-4">
      <div className="flex items-center justify-between mb-12">
        ...
        <div className="flex items-start gap-4">
          <span className="text-gray-400 text-sm mt-2">Signed in as {user.username}</span>
          <div className="flex flex-col items-end">
            <a
              href={inviteUrlFor()}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3.5 py-2 bg-cyan-500 hover:bg-cyan-400 text-[#0b1416] shadow-md shadow-cyan-500/20 transition rounded-lg text-sm font-medium shadow-lg shadow-cyan-500/20"
            >
              <UserPlus size={15} />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {readyGuilds.map((g) => (
              <button
                key={g.id}
                onClick={() => openGuild(g)}
                className="group relative bg-surface2/80 backdrop-blur-sm hover:bg-surface3 border border-white/5 hover:border-cyan-400/40 transition-all duration-200 rounded-2xl p-5 flex flex-col items-center gap-2.5 text-center hover:-translate-y-0.5 hover:shadow-xl hover:shadow-cyan-500/10"
              >
                <div className="w-16 h-16 rounded-full bg-surface3 ring-2 ring-white/10 group-hover:ring-cyan-400/50 transition flex items-center justify-center overflow-hidden">
                  {g.icon ? (
